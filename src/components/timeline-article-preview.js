...
export default ({ post, partial }) => (
  <div className={`tile ${styles.monthGroup}`}>
    <div className="tile-content columns">
      <div className="column col-3 col-sm-12">
      <Link to={`${partial}/${post.slug}`}><Img fluid={post.heroImage.fluid} fadeIn={true} /></Link>
      </div>
      <div className="column col-6 col-sm-12">
        <h5 className={`tile-title ${styles.timelinePreviewTitle}`}><Link to={`${partial}/${post.slug}`}>{post.title}</Link></h5>
